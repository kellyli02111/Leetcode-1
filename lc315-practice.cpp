...
// To execute C++, please define "int main()"
    ...
    void rotateRL(TreeNode* root, TreeNode* pre);
    void rotateLR(TreeNode* root, TreeNode* pre);
  ...
  TreeNode* buildAVL(vector<int>& nums){
    ...
    for(int i = 1; i < n; i++){
      ...
      for(int j = pathSize - 1; j >= 1; j--){
        int leftDepth = maxDepth(path[j]->left);
        int rightDepth = maxDepth(path[j]->right);
        if(leftDepth - rightDepth > 1){
          TreeNode* temp = path[j]->left;
          if(maxDepth(temp->left) >= maxDepth(temp->right)){
            rotateLL(path[j], path[j - 1]);
          } else {
            rotateLR(path[j], path[j - 1]);
          }
        }
        if(rightDepth - leftDepth > 1){
          TreeNode* temp = path[j]->right;
          if(maxDepth(temp->right) >= maxDepth(temp->left)){
            rotateRR(path[j], path[j - 1]);
          } else{
            rotateRL(path[j], path[j - 1]);
          }
        }
      }
    }
  ...
  void rotateRL(TreeNode* root, TreeNode* pre){
    rotateLL(root->right, root);
    rotateRR(root, pre);
  }

  void rotateLR(TreeNode* root, TreeNode* pre){
    rotateRR(root->left, root);
    rotateLL(root, pre);
  }
